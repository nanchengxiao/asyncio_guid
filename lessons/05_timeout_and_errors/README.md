...
如果时间耗尽，作用域内部会借助前一课学过的 cancellation 停止继续等待；越过 `asyncio.timeout()` 的边界后，调用者通常看到 `TimeoutError`。
...
- 失败是否可能只是暂时的；
- 重复执行是否安全；
- retry 会不会给已经过载的下游增加更多压力。

这些生产约束会在最后一课组合起来；本课只先建立规则：
...
```text
required failure → 当前业务整体不能成立
optional failure → 可以选择 degradation
```
...
- **误区：** `ExceptionGroup` 只需要打印。  
  **更准确：** 它让多个并行失败可以被同时保留和分类。

- **误区：** optional dependency 失败必须让整个请求失败。  
...
3. `asyncio.timeout()` 超时后调用者通常看到什么？
4. timeout 与 cancellation 的业务含义有什么不同？
5. required dependency 与 optional dependency 应由谁决定？
6. degradation 是什么？
7. 为什么并行 Task 可能需要 `ExceptionGroup`？
8. `except* ValueError` 做了什么？
9. 为什么 timeout 不能自动等价于 retry？

## 场景命题

实现一个 required operation 的 time budget，并实现一个并行失败收集函数。
...
- timeout 边界生效；
- required failure 没有被错误 degradation；
- 多个并行失败没有被错误丢失；
