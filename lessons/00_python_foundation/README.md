...
如果调用者只取前 3 条就停止，那么后面的记录可以根本不读取。本课练习说的“逐条按需产生记录”就是这个意思。
...
本课练习使用标准库提供的 `@contextmanager` 来帮助我们定义一个 context manager。你不需要理解 `@contextmanager` 背后的实现细节，只要先记住它的结构：
...
3. 无论正常结束还是调用者提前停止，离开 `with` 作用域时都会执行一次资源清理。
