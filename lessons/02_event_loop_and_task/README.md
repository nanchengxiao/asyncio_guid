...
它通常运行在一个 **thread（线程）** 里。这里先把线程理解成“程序的一条执行路径”即可，本课不要求掌握多线程；Lesson 09 会在同步阻塞代码的场景里再次用到它。

某段 Python 代码正在执行时，Event Loop 不会强行把它抢走；正在运行的异步工作需要在合适的位置主动让出执行机会，例如等待尚未完成的 I/O。
